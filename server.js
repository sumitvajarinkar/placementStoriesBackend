...
import serviceAccountKey from "./placementstories-web-app-firebase-adminsdk-n7ogx-a882cdebf2.json" assert { type: "json" };
...
const generateUploadURL = async () => {
  ...
  return await s3.getSignedUrlPromise("putObject", {
    Bucket: "placementstories-web-app",
    Key: imageName,
    Expires: 1000,
  ...
  User.findOne({ "personal_info.email": email })
    .then((user) => {
      ...
      if (!user.google_auth) {
        ...
        return res.status(403).json({
          error:
            "Account was created using Google. Try logging with Google below",
        });
      }
...
server.post("/create-blog", verifyJWT, (req, res) => {
  ...
  if (!title.length) {
    return res
      .status(403)
      .json({ error: "You must provide a title to a blog" });
  }

  if (!draft) {
    if (!des.length || des.length > 200) {
      return res.status(403).json({
        error: "You must provide experience description under 200 characters",
      });
    }
    ...
    if (!tags.length || tags.length > 10) {
      return res.status(403).json({
        error: "Provide tags in order to publish an experience, maximum 10",
      });
    }
  }
